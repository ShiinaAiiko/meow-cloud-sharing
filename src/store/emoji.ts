...
export const emojiMethods = {
	...
	>(modeName + '/init', async (_, thunkAPI) => {
		const { emoji, mwc } = thunkAPI.getState()
		const res = await mwc.sdk?.api.file.getCustomStickersFileUrl({})
		// console.log('emojiinit', res)

		const g = await axios.get(res?.data.url || '')
		if (g?.data?.list) {
			thunkAPI.dispatch(
				emojiSlice.actions.setCustomStickers(g?.data?.list || [])
			)
		}
	...
	addCustomSticker: createAsyncThunk<
		void,
		{
			csi: CustomStickersItem
		},
		{
			state: RootState
		}
	>(modeName + '/addCustomSticker', async ({ csi }, thunkAPI) => {
	...
	>(modeName + '/addCustomSticker', async ({ index }, thunkAPI) => {
		const { emoji } = thunkAPI.getState()
